...
def test_transcribe_to_subtitles_qwen3_backend_can_disable_forced_aligner(
    ...
    def fake_transcribe_chunks_qwen3(**_kwargs):
        assert _kwargs["return_time_stamps"] is False
        assert _kwargs["cfg"].forced_aligner == ""
    ...
    res = pipeline.transcribe_to_subtitles(
        ...
        qwen3_device="cpu",
        qwen3_max_inference_batch_size=4,
        qwen3_use_forced_aligner=False,
    )

    out_text = (tmp_path / res.subtitle_file_path.split("/")[-1]).read_text(encoding="utf-8")
    assert "00:00:00,000 --> 00:00:01,000" in out_text
